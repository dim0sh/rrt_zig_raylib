...
pub fn compileRaylib(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, options: Options) !*std.Build.Step.Compile {
    ...
    if (options.platform != .drm) {
        raylib.addIncludePath(b.path("raylib/src/external/glfw/include"));
    }

    var c_source_files = try std.ArrayList([]const u8).initCapacity(b.allocator, 2);
    c_source_files.appendSliceAssumeCapacity(&.{ "raylib/src/rcore.c", "raylib/src/utils.c" });

    if (options.rshapes) {
        try c_source_files.append("raylib/src/rshapes.c");
        try raylib_flags_arr.append("-DSUPPORT_MODULE_RSHAPES");
    }
    if (options.rtextures) {
        try c_source_files.append("raylib/src/rtextures.c");
        try raylib_flags_arr.append("-DSUPPORT_MODULE_RTEXTURES");
    }
    if (options.rtext) {
        try c_source_files.append("raylib/src/rtext.c");
        try raylib_flags_arr.append("-DSUPPORT_MODULE_RTEXT");
    }
    if (options.rmodels) {
        try c_source_files.append("raylib/src/rmodels.c");
        try raylib_flags_arr.append("-DSUPPORT_MODULE_RMODELS");
    }
    if (options.raudio) {
        try c_source_files.append("raylib/src/raudio.c");
    ...
    raylib.addIncludePath(b.path("raylib/src/platforms"));
    switch (target.result.os.tag) {
        .windows => {
            try c_source_files.append("raylib/src/rglfw.c");
